...
def run_inference(args):
    ...
    fig.suptitle("Rally-Segmented 3D Ball Trajectories", fontsize=14)
    ...
    for i, res in enumerate(results):
        ...
        ax4.plot(res["frames"], xyz[:, 1], "-", color=c, label=label, linewidth=1.5)
        # Mark EoT predictions
        eot_mask = res["eps"] > 0.5
        if eot_mask.any():
            ax4.scatter(res["frames"][eot_mask], xyz[eot_mask, 1],
                       color=c, s=60, marker="x", zorder=5)

    ax1.set_xlabel("X (m)"); ax1.set_ylabel("Z (m)"); ax1.set_zlabel("Y / height (m)")
    ax1.set_title("3D trajectory"); ax1.legend(fontsize=6)
    ...
    ax4.set_title("Height over time (x = EoT prediction)"); ax4.grid(True, alpha=0.3)
